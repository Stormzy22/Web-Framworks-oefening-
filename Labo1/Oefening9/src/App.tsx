...
const App =()=>{
const correctIndex:number = Math.floor(Math.random() * pokemons.length)
const correct:Pokemon = pokemons[correctIndex];

const wrongIndex:Set<number> = new Set<number>();
while(wrongIndex.size < 3){
  const i:number = Math.floor(Math.random() * pokemons.length);
  if(i !== correctIndex)
    wrongIndex.add(i);
}

// Foute opties met array (geen Set)
  // const wrongIndexes: number[] = [];
  // while (wrongIndexes.length < 3) {
  //   const i: number = Math.floor(Math.random() * pokemons.length);
  //   // voeg alleen toe als het niet de correcte index is en nog niet in wrongIndexes zit
  //   if (i !== correctIndex && !wrongIndexes.includes(i)) {
  //     wrongIndexes.push(i);
  //   }
  // }
...
const options:Pokemon[] = [
  correct,
  ...Array.from(wrongIndex).map((i:number)=> pokemons[i]),
];

// opties-array
  //  const options: Pokemon[] = [correct, ...wrongIndexes.map(i => pokemons[i])];


options.sort(() => Math.random() - 0.5);
  return(
    <div>
      <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${correct.id}.png `}alt={correct.name} />
      <div>
        {options.map((p:Pokemon) => (
